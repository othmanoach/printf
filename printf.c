 ...
 */
int helper_func(char specifier, va_list input, int flag_plus, int flag_space, int flag_hash)
{
    int len = 0;
    ...
    {
        unsigned int unum = va_arg(input, unsigned int);
        if (flag_plus)
            len += print_plus_unsigned_int(unum);
        else if (flag_space)
            len += print_space_unsigned_int(unum);
        else
